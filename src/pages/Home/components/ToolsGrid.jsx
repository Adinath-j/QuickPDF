...
import { Layers, SplitSquareHorizontal, Stamp, Image as ImageIcon } from 'lucide-react';
...
export function ToolsGrid() {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6 w-full max-w-6xl mx-auto mb-32">
      ...
      {/* 4. Image to PDF Card */}
      <motion.div variants={cardVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} transition={{ delay: 0.3 }}>
        <Link to="/image-to-pdf" className="group flex flex-col p-8 bg-[#0a0a0a] border border-white/10 rounded-3xl hover:border-white/30 hover:bg-white/[0.02] hover:shadow-[0_0_40px_rgba(255,255,255,0.03)] transition-all duration-500 text-left h-full relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-white/[0.02] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
          <div className="relative z-10 w-14 h-14 border border-white/10 bg-zinc-900 text-white rounded-2xl flex items-center justify-center mb-8 group-hover:scale-110 group-hover:bg-white group-hover:text-black transition-all duration-500">
            <ImageIcon className="w-6 h-6" />
          </div>
          <h2 className="relative z-10 text-2xl font-semibold text-white mb-3 tracking-tight">Image to PDF</h2>
          <p className="relative z-10 text-zinc-400 mb-8 font-light flex-grow leading-relaxed">
