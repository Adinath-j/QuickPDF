...
export function ImageToPdf() {
  ...
  const handleFilesSelected = (selectedFiles) => {
    ...
    if (validImages.length !== selectedFiles.length) {
      setError(
        "Some files were ignored. Only JPG and PNG images are supported.",
      );
    } else {
      setError(null);
    }
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setError(
        err.message ||
          "An error occurred while converting the images. Please try again.",
      );
    } finally {
  ...
  return (
    ...
    >
      <div className="text-center mb-10">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-zinc-900 border border-white/10 text-white mb-4 shadow-[0_0_30px_rgba(255,255,255,0.03)]">
        ...
        <p className="text-lg text-zinc-400">
          Convert JPG and PNG images into a PDF document. Drag to reorder your
          pages.
        </p>
      </div>
      ...
      <div className="bg-[#0a0a0a] rounded-2xl border border-white/10 p-6 md:p-8 shadow-2xl">
        {error && (
          <motion.div
            ...
            className="mb-6 p-4 bg-red-500/10 text-red-400 rounded-lg text-sm border border-red-500/20"
          >
            {error}
          </motion.div>
        )}
        ...
        <div className="mb-8">
          <Dropzone
            ...
            text="Click to upload JPG or PNG files"
            accept="image/jpeg, image/png, image/jpg" // Forces file picker to show images
            hintText="JPG, PNG Files Only" // Updates the UI text
          />
        </div>
