...
export function Compress() {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `QuickPDF_Compressed_${file.name}`;
    a.click();
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                {options.map((opt) => (
                  <button
                    ...
                    onClick={() => {
                      setLevel(opt.id);
                      setResult(null); // Clear previous result if they change levels
                    }}
                    disabled={isProcessing}
                    className={`flex flex-col items-center justify-center p-4 rounded-xl border transition-all disabled:opacity-50 ${
                      level === opt.id
                        ? "bg-white/10 border-white/30 text-white"
                        : "bg-zinc-900/30 border-white/5 text-zinc-400 hover:bg-zinc-900/60 hover:border-white/15"
                    }`}
                  >
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <span className="text-emerald-500/80 text-xs mt-0.5">
                      Saved {result.savings}% • New size: {formatFileSize(result.size)}
                    </span>
                  </div>
